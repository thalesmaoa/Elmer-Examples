...
if len(sys.argv) > 2:
    pvd_file_path = sys.argv[1]
    output_screenshot_path = sys.argv[2]
else:
    # Fallback to default paths if no arguments are provided
    pvd_file_path = './sim/result/paraview/results.pvd'
    output_screenshot_path = './sim/plots/cena_paraview.png'
...
resultspvd = PVDReader(registrationName='results.pvd', FileName=pvd_file_path)
resultspvd.Set(
...
import os
output_dir = os.path.dirname(output_screenshot_path)
if output_dir and not os.path.exists(output_dir):
    os.makedirs(output_dir)

SaveScreenshot(output_screenshot_path)
#
