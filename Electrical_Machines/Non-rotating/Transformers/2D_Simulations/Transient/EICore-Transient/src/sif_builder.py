...
def setup_simulation_sif(mesh_basename, physical_ids, core_depth=70e-3, N_turns=217):
    ...
    """
    # --- Simulation Setup ---
    sim = elmer.load_simulation("2D_transient")
    
    # Solução para as pastas: Caminho absoluto para a malha e diretório local para resultados
    import os
    mesh_dir_abs = os.path.abspath("mesh")
    sim.header["Mesh DB"] = f'"{mesh_dir_abs}" "{mesh_basename}"'
    sim.header["Results Directory"] = '"."'
    ...
    frequency = 60
    period = 1/frequency
    time_step = period / n_simulation

    sim.settings["Timestep Sizes"] = time_step
    sim.settings["Timestep Intervals"] = n_simulation
    sim.settings["Output Intervals"] = 1
    
    solver_magnetic_paraview.data["Exec Solver"] = "After timestep"
    solver_magnetic_paraview.data["Vtu Time Collection"] = "Logical True"
